<?php
	...
	if ($_POST['phpFunction'] == 'updateBike') {
		updateBike();
	}
	...
	function updateBike() {
		...
		*/
		...
		$sql = "UPDATE `bikesTable` SET `nickname`='" . $nickname . "', `manufacturer`='" . $manufacturer . "', `model`='" . $model . "', `type`='" . $type . "', `mpn`='" . $mpn . "', `colour`='" . $colour . "', `wheelSize`='" . 
		$wheelSize . "', `numberOfGears`='" . $numberOfGears . "', `typeOfBreaks`='" . $typeOfBreaks . "', `suspension`='" . $suspension . "', `gender`='" . $gender . "', `ageGroup`='" . $ageGroup . "', `commonParkingPlace`='" . 
		$commonParkingPlace . "', `useCase`='" . $useCase . "', `otherDetails`='" . $otherDetails . "' WHERE `bikeID`='" . $bikeID . "' AND `ownerID`='" . $ownerID . "'";
