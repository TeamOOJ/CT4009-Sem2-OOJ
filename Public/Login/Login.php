...
if (empty($_SESSION['currentUser'])) {
    ...
    if (!$res = $mysqli->query("SELECT * FROM `usersTable` WHERE `userName` = '$userName' LIMIT 1")) { # read the database entry for the given username
        echo "Error: (" . $mysqli->errno . ") " . $mysqli->error;
        exit;
    } else {
        $row = $res->fetch_assoc();

        if (!$userName == $row['userName']) { # check if the username exists in the database, if not, throw an error.
            destroySession();
        ...
        if (password_verify($password, $row['password'])) { # check that the entered password matches the encrypted one on file
            $_SESSION['currentUser'] = $userName; # it matches
            loggedIn();
        } else {
            # it doesn't match
    ...
    # Check that the username in the session matches one on file
    ...
    if ($res = $mysqli->query("SELECT * FROM `usersTable` WHERE `userName` = '" . $_SESSION['currentUser'] . "' LIMIT 1")) {
        $row = $res->fetch_assoc();
